...
int main(void)
{
	...
	scanf("%d", &n);
		...
		cl_mem a_mem_obj = clCreateBuffer(context,
			CL_MEM_READ_WRITE, n* sizeof(int), NULL, &ret);
		...
		ret = clEnqueueWriteBuffer(command_queue, a_mem_obj, CL_TRUE, 0, n
		...
		cl_kernel kernel = clCreateKernel(program, "linear1", &ret);
		// Set the arguments of the kernel
		ret = clSetKernelArg(kernel, 0, sizeof(cl_mem), (void *)&a_mem_obj);
		ret = clSetKernelArg(kernel, 1, sizeof(int), &ele);
		...
		ret = clEnqueueReadBuffer(command_queue, b_mem_obj, CL_TRUE, 0, sizeof(int), B, 0, NULL, NULL);
